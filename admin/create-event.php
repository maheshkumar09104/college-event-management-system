...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        if (!$title || !$event_date) {
            ...
            $stmt = $conn->prepare("INSERT INTO events (title, description, category, venue, event_date, event_time, max_participants, registration_deadline, banner_color, poster_image, registration_link, contact_name, contact_email, contact_phone, status, created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->bind_param("ssssssissssssssi", $title, $description, $category, $venue, $event_date, $event_time, $max_part, $deadline, $banner_color, $poster_name, $reg_link, $contact_name, $contact_email, $contact_phone, $status, $user_id);
            if ($stmt->execute()) {
...
<title>Create Event – <?= SITE_NAME ?></title>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .poster-upload:hover { border-color: var(--accent); background: rgba(59,130,246,0.04); }
  ...
  .poster-upload span { color: var(--accent); font-weight: 600; }
...
<aside class="sidebar">
  ...
  <a href="create-event.php" class="sidebar-link active"><span>➕</span> Create Event</a>
...
<div class="main">
  ...
  <?php if ($success): ?><div class="alert alert-success">✅ <?= $success ?> <a href="dashboard.php" style="color:inherit;font-weight:700">Go to Dashboard →</a></div><?php endif; ?>

  <div class="form-card">
    <form method="POST" enctype="multipart/form-data">
      <div class="section-title">📋 Basic Information</div>
      <div class="field"><label>Event Title *</label><input type="text" name="title" placeholder="e.g., Tech Symposium 2026" required value="<?= $_POST['title'] ?? '' ?>"></div>
      <div class="field"><label>Description</label><textarea name="description" placeholder="Describe the event..."><?= $_POST['description'] ?? '' ?></textarea></div>
      <div class="grid-2">
        <div class="field"><label>Category</label>
          <select name="category"><?php foreach ($categories as $c): ?><option value="<?= $c ?>" <?= (($_POST['category'] ?? '')===$c)?'selected':'' ?>><?= $c ?></option><?php endforeach; ?></select>
        </div>
        <div class="field"><label>Status</label>
          <select name="status"><option value="upcoming">Upcoming</option><option value="ongoing">Ongoing</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select>
        </div>
      </div>
      ...
      <div class="grid-2">
        <div class="field"><label>Max Participants</label><input type="number" name="max_participants" min="1" value="100"></div>
        <div class="field"><label>Registration Deadline</label><input type="date" name="registration_deadline"></div>
      </div>
      <div class="field"><label>External Registration Link (optional)</label><input type="url" name="registration_link" placeholder="https://forms.google.com/... (leave empty for in-app registration)"></div>
      ...
      <div class="color-picker">
        ...
        <label class="color-dot <?= $i===0?'selected':'' ?>" style="background:<?= $c ?>">
          <input type="radio" name="banner_color" value="<?= $c ?>" <?= $i===0?'checked':'' ?>>
        </label>
        <?php endforeach; ?>
      </div>
