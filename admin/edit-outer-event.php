...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE outer_events SET title=?,description=?,organizing_college=?,category=?,venue=?,event_date=?,event_time=?,registration_deadline=?,registration_link=?,contact_name=?,contact_email=?,contact_phone=?,poster_image=?,status=? WHERE id=?");
    $stmt->bind_param("ssssssssssssssi", $title,$description,$org_college,$category,$venue,$event_date,$event_time,$deadline,$reg_link,$contact_name,$contact_email,$contact_phone,$poster_name,$status,$id);
    if ($stmt->execute()) {
...
<title>Edit Outer Event – <?= SITE_NAME ?></title>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .poster-upload:hover { border-color: var(--accent2); }
  ...
  .current-poster { width: 100%; max-height: 160px; object-fit: cover; border-radius: 10px; margin-bottom: 10px; }
  #posterPreview { width: 100%; max-height: 160px; object-fit: cover; border-radius: 10px; margin-top: 10px; display: none; }
...
<div class="main">
  ...
  <div class="form-card">
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="field"><label>Organizing College *</label><input type="text" name="organizing_college" required value="<?= htmlspecialchars($event['organizing_college']) ?>"></div>
      ...
      <div class="grid-2">
        ...
        <div class="field"><label>Registration Link</label><input type="url" name="registration_link" value="<?= htmlspecialchars($event['registration_link']) ?>"></div>
      </div>
      ...
      <div class="poster-upload">
        ...
        <?php if ($event['poster_image']): ?>
          ...
          <p>Current poster shown above. Click to replace.</p>
        <?php else: ?>
